...
function showSuccessMessage(total) {
  ...
  if (successTotal) {
    successTotal.textContent =
      "Sipariş toplamı: " + total + " TL";
  }
  if (successMessage) {
    successMessage.classList.add("active");
    document.body.style.overflow = "hidden";
  } else {
    // Eğer HTML eklenmemişse
    // en azından kullanıcıya bilgi ver.
    alert(
...
async function submitOrder(event) {
  ...
  try {
    ...
    if (response.ok) {
      // Önce sipariş formunu ve sepeti kapat
      closeOrderForm();
      closeCart();
      // Formu temizle
      const form =
      ...
      showSuccessMessage(order.total);
    } else {
      let errorMessage =
      ...
      try {
        ...
        console.log(
          "Formspree hata detayı alınamadı."
        );
      }
      alert(
        errorMessage +
    ...
    alert(
      "İnternet bağlantısında bir sorun oluştu.\n\n" +
      "Lütfen tekrar deneyin."
    );
  }
  if (submitButton) {
